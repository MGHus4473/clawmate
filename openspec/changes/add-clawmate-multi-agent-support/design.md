# Design: Multi-agent support for clawmate-companion
...
### 1. Runtime context is the source of truth
...
`api.config.agents.list` 只可作为 fallback 或诊断用途，不作为主流程。

### 2. Per-agent config overrides are plugin-defined
...
### 3. SOUL injection follows workspaceDir
...
### 4. Tool state isolation uses two lifecycles
...
### 5. Session state cleanup is explicit
